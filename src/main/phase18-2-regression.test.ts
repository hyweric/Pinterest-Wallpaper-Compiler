...
test("resize dots have large hit targets while keeping compact visible markers", async () => {
  ...
  assert.match(styles, /\.selection-handles-overlay \.resize-handle \{[\s\S]*width: var\(--handle-hit, 28px\);[\s\S]*height: var\(--handle-hit, 28px\);/);
  assert.match(styles, /\.selection-handles-overlay \.resize-handle::after \{[\s\S]*width: var\(--handle-dot, 12px\);[\s\S]*height: var\(--handle-dot, 12px\);/);

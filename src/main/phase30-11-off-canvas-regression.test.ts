...
test("editor clips artwork with a single rotation-safe viewport while leaving controls unclipped", async () => {
  ...
  assert.match(css, /Phase 30\.16: rotation-safe canvas clipping/);
  assert.match(sourceCode, /className="canvas-artwork-clip"/);
  assert.doesNotMatch(sourceCode, /canvasArtworkClipMaskStyle/);
  assert.doesNotMatch(sourceCode, /CanvasOutsideClipMask/);
  assert.match(css, /\.canvas-artwork-clip \{[\s\S]*overflow:\s*hidden/);

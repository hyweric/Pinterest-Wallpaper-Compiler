...
test("phase 22.3 quick canvas toolbar stays compact", async () => {
  const renderer = await readFile(path.join(process.cwd(), "src/renderer/main.tsx"), "utf8");
...
test("phase 22.3 effects tab has three simplified sections", async () => {
  const renderer = await readFile(path.join(process.cwd(), "src/renderer/main.tsx"), "utf8");
  assert.match(renderer, /<summary>Paper <ChevronDown size=\{15\} \/><\/summary>/);
  assert.match(renderer, /<summary>Frame Style <ChevronDown size=\{15\} \/><\/summary>/);
  assert.match(renderer, /<summary>Shadow and Blend <ChevronDown size=\{15\} \/><\/summary>/);
  assert.match(renderer, /Choose Clean Paper, Polaroid, or Torn Paper to show frame controls/);
});
